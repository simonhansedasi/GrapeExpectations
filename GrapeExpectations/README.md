...
| `build_ndvi_temporal_gpkg.py` / `qgis_elevation_ndvi_lawn.py` | **Experimental, abandoned 2026-08-20** — a QGIS Temporal Controller "NDVI as growing grass" animation. Works but was dropped in favor of the interactive Leaflet map below (33k+ individually-rendered markers was the wrong direction regardless of speed tuning). Kept in the repo as a working example of PyQGIS temporal-layer scripting; not part of the live site. Full debugging history in `CONTEXT.md`. |
...
**`ndvi_interactive.geojson`** (the interactive map's data, also copied to `site/data/`) was generated ad hoc from `grapeexpectations.gpkg` during the 2026-08-20/21 session — no persisted script for it yet, just the commands logged in `CONTEXT.md`. Write one if this needs regenerating for a new year or field set.

## Public report site (`site/`)

Static page at `grape-expectations.simonhansedasi.com`. Leads with a live **interactive Leaflet map** (`site/map.html`, all 33,028 cells client-side, toggle between NDVI snapshot / 9-year trend / elevation / slope) and a live embedded **Looker Studio BI report**, both above three branch cards: §1 Pipeline (ingestion, feature engineering, model training/evaluation, spatial residual diagnostics), §2 GIS (QGIS export + toggle between two static graduated-symbology maps + matplotlib fallback — the interactive map above supersedes/extends this to four layers), §3 BI (the embedded live report + star-schema CSV downloads). Deploy with `site/deploy_hetzner.sh` (rsyncs to the Hetzner box, no DNS change needed — `*.simonhansedasi.com` is already wildcarded there, and `.ipynb_checkpoints/` is excluded from the sync). `site/nginx_grape_expectations.conf` is a reference config; the live server block's cert directives should be copied from an already-working simonhansedasi.com subdomain rather than assumed from this file.

**Live and current as of 2026-08-21** — https://grape-expectations.simonhansedasi.com. §3 (BI/Looker) is done and embedded, no longer in-progress. Re-run `site/deploy_hetzner.sh` for content updates; nginx/cert are already provisioned. See `CONTEXT.md` for the full debugging history behind the current layout (a page-wide scrollbar bug from a `100vw` CSS trick, a bad SRI hash that broke Leaflet's own CSS, two Looker report-ID typos, an aggregation bug in the BI leaderboard) — worth reading before assuming any current sizing/positioning choice was arbitrary.
